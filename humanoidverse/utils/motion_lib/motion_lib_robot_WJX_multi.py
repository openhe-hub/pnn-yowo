...
class MotionLibBase():
    ...
    def load_motions(self, 
        ...
        # import ipdb; ipdb.set_trace() 
        res_acc = {i: (res_acc_single[i]) for i in range((num_motion_to_load))}
        ...
        # 使用 numpy.concatenate 拼接numpy数组，然后转换为tensor
        ...
        # 使用列表推导式从每个motion对象中提取属性，然后用torch.cat拼接
        ...
        if flags.real_traj:
            # 这里self.q_gts等是tensor列表，直接用torch.cat拼接
            self.q_gts = torch.cat(self.q_gts, dim=0).float().to(self._device)
            # q_qts 似乎是 q_gts 的笔误或副本，这里我们直接使用拼接好的 q_gts 即可
            self.q_qts = self.q_gts 
    ...
    def load_motion_with_skeleton(self,
        ...
        res = {}
        print(f"len = {len(motion_data_list)}")
        for f in track(range(len(motion_data_list)), description="Loading motions..."):
            ...
            if not isinstance(curr_file, dict) and osp.isfile(curr_file):
                ...
                print(f"key = {key}, curr_file = {curr_file}")
                curr_file = joblib.load(curr_file) 
                curr_file = curr_file[list(curr_file.keys())[0]]
