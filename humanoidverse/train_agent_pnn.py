...
def main(config: OmegaConf):
    ...
    # pnn 
    tasks = config.pnn.motions
    logger.critical(tasks)
    
    config.robot.motion.motion_file = tasks[0]
    pre_process_config(config)
    env: BaseEnv = instantiate(config=config.env, device=device)
    algo: BaseAlgo = instantiate(device=device, env=env, config=config.algo, log_dir=experiment_save_dir, num_tasks=len(tasks))
    algo.setup()
    
    for task_id, motion_path in enumerate(tasks):
        logger.critical(f"task id = {task_id}, motion = {motion_path}")
        if task_id != 0:
            config.robot.motion.motion_file = motion_path
        algo.new_task(task_id, config)
