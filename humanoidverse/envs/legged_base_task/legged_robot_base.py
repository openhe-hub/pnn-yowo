...
class LeggedRobotBase(BaseTask):
    ...
    def _reset_buffers_callback(self, env_ids, target_buf=None):
        if target_buf is not None:
            ...
            self.last_actions = self.last_actions.clone()
            self.actions_after_delay = self.actions_after_delay.clone()
            self.contacts = self.contacts.clone()
            self.contacts_filt = self.contacts_filt.clone()
            self.last_contacts = self.last_contacts.clone()
            self.last_contacts_filt = self.last_contacts_filt.clone()
