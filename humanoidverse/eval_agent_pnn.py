...
def main(override_config: OmegaConf):
    ...
    logger.critical(task_id)
    logger.critical(len(config.pnn.motions))
    config.robot.motion.motion_file = config.pnn.motions[config.motion_id]
    pre_process_config(config)
    logger.critical(config.robot.motion.motion_file)
    ...
    algo: BaseAlgo = instantiate(config.algo, env=env, device=device, log_dir=None, num_tasks=len(config.pnn.motions))
    algo.setup()
    for i in range(len(config.pnn.motions)):
        algo.new_task(i, config, eval=True)
